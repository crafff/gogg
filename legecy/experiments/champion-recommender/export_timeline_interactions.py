#!/usr/bin/env python3
"""Export jungle interactions enriched with 10/15-minute snapshots."""
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("data/jungle_timeline_interactions.csv"))
    ...
    sql = Path(__file__).with_name("extract_timeline_interactions.sql").read_text(encoding="utf-8")
    params = {
    ...
    print(f"exported {len(frame)} timeline interactions for {frame.puuid.nunique()} players")
