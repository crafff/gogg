...
func (q *Queries) ListTFTTeamPlannerUnits(ctx context.Context, snapshotID int64, setID string) ([]ListTFTTeamPlannerUnitsRow, error) {
	rows, err := q.db.Query(ctx, listTFTTeamPlannerUnits, snapshotID, setID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListTFTTeamPlannerUnitsRow{}
	for rows.Next() {
		var i ListTFTTeamPlannerUnitsRow
		if err := rows.Scan(
			&i.SetID,
			&i.CharacterID,
			&i.PlannerCode,
...
const upsertTFTStaticTeamPlannerUnit = `-- name: UpsertTFTStaticTeamPlannerUnit :exec
...
type UpsertTFTStaticTeamPlannerUnitParams struct {
	SnapshotID  int64
	SetID       string
	CharacterID string
	PlannerCode int32
...
func (q *Queries) UpsertTFTStaticTeamPlannerUnit(ctx context.Context, arg UpsertTFTStaticTeamPlannerUnitParams) error {
	_, err := q.db.Exec(ctx, upsertTFTStaticTeamPlannerUnit,
		arg.SnapshotID,
		arg.SetID,
		arg.CharacterID,
		arg.PlannerCode,
