...
-- name: UpsertTFTStaticTeamPlannerUnit :exec
INSERT INTO tft_static_team_planner_units (
    snapshot_id, set_id, character_id, planner_code, cost, payload
) VALUES (
    @snapshot_id, @set_id, @character_id, @planner_code, sqlc.narg(cost), @payload
)
ON CONFLICT (snapshot_id, set_id, character_id) DO UPDATE
SET planner_code = EXCLUDED.planner_code,
    cost = COALESCE(EXCLUDED.cost, tft_static_team_planner_units.cost),
    payload = EXCLUDED.payload;
...
-- name: ListTFTTeamPlannerUnits :many
SELECT units.set_id, units.character_id, units.planner_code, units.cost
FROM tft_static_team_planner_units units
WHERE units.snapshot_id = @snapshot_id
  AND units.set_id = @set_id
ORDER BY units.character_id;
