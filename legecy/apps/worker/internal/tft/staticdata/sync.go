...
const parserVersion = "tft-static-v3"
...
var cdragonClientFiles = []string{
	"tftsets.json",
	"tftchampions.json",
	"tftchampions-teamplanner.json",
	"tfttraits.json",
	"tftitems.json",
	"tftregionportals.json",
}
...
type Querier interface {
	...
	UpsertTFTStaticTeamPlannerUnit(context.Context, sqlcgen.UpsertTFTStaticTeamPlannerUnitParams) error
...
func persistDocuments(ctx context.Context, q Querier, opts Options, snapshotID int64, source, patch, revision, locale, build string, docs []document) (int, error) {
	...
	for _, doc := range docs {
		...
		if source == "cdragon" && doc.kind == "tftchampions-teamplanner" {
			units, err := parseTeamPlannerDocument(doc.body)
			if err != nil {
				return 0, fmt.Errorf("parse TFT static document %s: %w", doc.name, err)
			}
			plannerUnits = append(plannerUnits, units...)
			continue
		}
		assetVersion := build
		if source == "cdragon" {
	...
	for _, unit := range plannerUnits {
		if err := q.UpsertTFTStaticTeamPlannerUnit(ctx, sqlcgen.UpsertTFTStaticTeamPlannerUnitParams{
			SnapshotID: snapshotID,
			SetID:      unit.setID, CharacterID: unit.characterID,
			PlannerCode: int32(unit.plannerCode), Cost: unit.cost,
...
type teamPlannerUnit struct {
	setID, characterID string
	plannerCode        int
	cost               *int32
	payload            []byte
}

func parseTeamPlannerDocument(body []byte) ([]teamPlannerUnit, error) {
	var root map[string][]map[string]any
	if err := json.Unmarshal(body, &root); err != nil {
		return nil, err
	}
	setIDs := make([]string, 0, len(root))
	for setID := range root {
		setIDs = append(setIDs, setID)
	}
	sort.Strings(setIDs)
	units := []teamPlannerUnit{}
	seenCharacters := map[string]bool{}
	seenCodes := map[string]bool{}
	for _, setID := range setIDs {
		if !strings.HasPrefix(setID, "TFTSet") {
			continue
		}
		for _, raw := range root[setID] {
			characterID := stringField(raw, "character_id")
			plannerCode := number(raw["team_planner_code"])
			if characterID == "" || plannerCode == 0 {
				continue
			}
			if plannerCode < 0 || plannerCode > 0xfff {
				return nil, fmt.Errorf("planner code %d for %s/%s is outside 12-bit range", plannerCode, setID, characterID)
			}
			characterKey := setID + "\x00" + characterID
			codeKey := fmt.Sprintf("%s\x00%d", setID, plannerCode)
			if seenCharacters[characterKey] || seenCodes[codeKey] {
				return nil, fmt.Errorf("duplicate team planner mapping for %s/%s code %d", setID, characterID, plannerCode)
			}
			seenCharacters[characterKey], seenCodes[codeKey] = true, true
			payload, _ := json.Marshal(raw)
			var cost *int32
			if tier := int32(number(raw["tier"])); tier > 0 {
				cost = &tier
			}
			units = append(units, teamPlannerUnit{
				setID: setID, characterID: characterID,
				plannerCode: plannerCode, cost: cost, payload: payload,
			})
		}
	}
	if len(units) == 0 {
		return nil, errors.New("team planner catalog contains no importable units")
	}
	return units, nil
}
