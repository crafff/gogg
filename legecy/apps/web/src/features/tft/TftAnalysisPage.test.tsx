          ...
          data: {
            tftObservedLineups: {
              ...
              items: [
                {
                  id: "lineup-1",
                  teamCode: "023ef439000000000000000000000000TFTSet18",
                  coreUnits: [
                    {
                      id: "DA_18_Aphelios",
                      name: "厄斐琉斯",
...
describe("TftAnalysisPage observed preview", () => {
  it("renders real-run provenance, coverage, and lineup cards", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: /厄斐琉斯.*霞/ }),
    ).not.toBeInTheDocument();
    expect(screen.getByText("2.62")).toBeInTheDocument();
    expect(screen.getByTitle("鬼索的狂暴之刃")).toBeInTheDocument();
    expect(screen.getByText(/3-star:.*97%|三星：.*97%/)).toBeInTheDocument();
    expect(
      screen.getByLabelText(/厄斐琉斯.*(?:3-star|三星)/),
    ).toBeInTheDocument();
    expect(screen.getAllByText(/Core|核心/).length).toBeGreaterThan(0);
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Copy team code|复制阵容码/ }),
    );
    await waitFor(() => {
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
        "023ef439000000000000000000000000TFTSet18",
      );
    });
  ...
  it("reports a copy failure when the Clipboard API is unavailable", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", {
        name: /Copy team code|复制阵容码/,
      }),
    );

    expect(
      (await screen.findAllByText(/Copy failed|复制失败/)).length,
    ).toBeGreaterThan(0);
  });
});
