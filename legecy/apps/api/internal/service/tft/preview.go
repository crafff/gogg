...
const (
	...
	ObservedPreviewAlgorithm       = "exact-observed-board-items-stars-presentation-v4"
...
type rawObservedDetails struct {
	...
	Augments                    []rawObservedItem            `json:"augments"`
...
func (s *Service) ObservedLineups(ctx context.Context, filter ObservedFilter) (*ObservedResult, error) {
	...
	localized, err := s.q.ListLatestTFTLocalizedStaticObjects(
		ctx,
		base.row.CatalogSnapshotID,
		[]string{"unit", "item", "augment"},
		filter.Locale,
	)
	if err != nil {
	...
	if row.SetNumber != nil && assets != nil {
		plannerUnits, plannerErr := s.q.ListTFTTeamPlannerUnits(
			ctx,
			assets.ID,
			fmt.Sprintf("TFTSet%d", *row.SetNumber),
		)
		if plannerErr != nil {
			slog.WarnContext(ctx, "TFT team planner mapping unavailable", "snapshot_id", assets.ID, "error", plannerErr)
		} else {
			applyPlannerUnits(entities, plannerUnits)
		}
	}
	...
	for _, item := range selected {
		...
		for _, augment := range detail.Augments {
			lineup.CommonAugments = append(lineup.CommonAugments, Count{
				Entity: localizedEntity(entities, augment.ID),
				Count:  int(augment.Count), Rate: augment.Rate,
			})
		}
		sortEntitiesByCost(lineup.CoreUnits)
		sort.SliceStable(lineup.UnitItems, func(i, j int) bool {
			left, right := []Entity{lineup.UnitItems[i].Unit}, []Entity{lineup.UnitItems[j].Unit}
			sortEntitiesByCost(left)
			sortEntitiesByCost(right)
			leftCost, rightCost := 99, 99
			if left[0].Cost != nil {
				leftCost = *left[0].Cost
			}
			if right[0].Cost != nil {
				rightCost = *right[0].Cost
			}
		...
		lineup.TeamCode = encodeTeamPlannerCode(result.SetNumber, lineup.CoreUnits)
		lineup.Signals = observedSignals(lineup)
...
func indexLatestLocalizedEntities(rows []sqlcgen.ListLatestTFTLocalizedStaticObjectsRow) (map[string]Entity, *ObservedStaticSnapshot) {
	...
	for _, row := range rows {
		if snapshot == nil {
			snapshot = &ObservedStaticSnapshot{ID: row.SnapshotID, Source: "cdragon", Patch: row.Patch, Revision: row.Revision}
		}
		entity := Entity{
			ID: row.ObjectID, IconURL: localTFTIconURL(row.Payload, row.Patch, row.Revision),
			ItemClass: itemClass(row.ObjectKind, row.ObjectID),
		}
		if row.Name != nil {
