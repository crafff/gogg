...
func (q *observedQuerier) GetTFTObservedLineupDetails(ctx context.Context, params sqlcgen.GetTFTObservedLineupDetailsParams) (interface{}, error) {
	...
	for _, signature := range params.Signatures {
		details = append(details, map[string]any{
			"id": signature,
			"augments": []map[string]any{{
				"id": "TFT18_Augment_Special", "count": 25, "rate": 25.0 / 58.0,
			}},
...
func (q *observedQuerier) ListLatestTFTLocalizedStaticObjects(_ context.Context, _ int64, kinds []string, _ string) ([]sqlcgen.ListLatestTFTLocalizedStaticObjectsRow, error) {
	q.localizedKinds = append([]string(nil), kinds...)
	name := "Aphelios"
	itemName := "Guinsoo's Rageblade"
	augmentName := "Special Delivery"
	unitCost := int32(2)
	return []sqlcgen.ListLatestTFTLocalizedStaticObjectsRow{{
		...
		SnapshotID: 73,
		ObjectKind: "augment", ObjectID: "TFT18_Augment_Special", Name: &augmentName,
		Payload: []byte(`{"iconPath":"/lol-game-data/assets/ASSETS/Maps/TFT/Icons/Augments/Special.png"}`),
		Patch:   "16.17", Revision: "static-rev",
	}}, nil
}

func (q *observedQuerier) ListTFTTeamPlannerUnits(_ context.Context, snapshotID int64, setID string) ([]sqlcgen.ListTFTTeamPlannerUnitsRow, error) {
	q.plannerSnapshot = snapshotID
	if q.plannerErr != nil {
		return nil, q.plannerErr
	}
	return []sqlcgen.ListTFTTeamPlannerUnitsRow{
		{SetID: setID, CharacterID: "DA_18_Aphelios", PlannerCode: 1007},
		{SetID: setID, CharacterID: "DA_18_Xayah", PlannerCode: 1081},
		{SetID: setID, CharacterID: "DA_18_Yasuo", PlannerCode: 1082},
	}, nil
}
...
func TestObservedLineupsKeepsMatchPatchUnknownAndLabelsReferenceAssets(t *testing.T) {
	...
	require.Equal(t, int64(73), q.plannerSnapshot)
	require.Equal(t, int64(6838), result.UsableParticipants)
	require.Len(t, result.Items, 2)
	require.Equal(t, "Aphelios", result.Items[0].CoreUnits[0].Name)
	require.Equal(t, 2, *result.Items[0].CoreUnits[0].Cost)
	require.Equal(t, "DA_18_Xayah", result.Items[0].CoreUnits[1].ID)
	require.Empty(t, result.Items[0].CoreUnits[1].Name)
	require.NotNil(t, result.Items[0].TeamCode)
	require.Equal(t, "023ef439000000000000000000000000TFTSet18", *result.Items[0].TeamCode)
	require.Len(t, result.Items[0].Signals, 2)
	require.Equal(t, TFTSignalThreeStar, result.Items[0].Signals[0].Kind)
	require.Equal(t, TFTSignalAugment, result.Items[0].Signals[1].Kind)
	require.Equal(t, "Special Delivery", result.Items[0].Signals[1].Entity.Name)
	require.Equal(t, []string{"unit", "item", "augment"}, q.localizedKinds)
...
func TestObservedLineupsDegradesWhenPlannerMetadataIsUnavailable(t *testing.T) {
	q := &observedQuerier{plannerErr: pgx.ErrNoRows}
	service := New(q, nil, nil, RuntimeConfig{})
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, result.Items[0].TeamCode)
}
