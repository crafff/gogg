...
func mapTFTLineup(item tft.Lineup) *gqlgenerated.TFTLineup {
	return &gqlgenerated.TFTLineup{
		...
		Signals:                       mapTFTSignals(item.Signals),
...
func mapTFTSignals(items []tft.Signal) []*gqlgenerated.TFTLineupSignal {
	out := make([]*gqlgenerated.TFTLineupSignal, 0, len(items))
	for _, item := range items {
		var holder *gqlgenerated.TFTEntity
		if item.HolderUnit != nil {
			holder = mapTFTEntities([]tft.Entity{*item.HolderUnit})[0]
		}
		out = append(out, &gqlgenerated.TFTLineupSignal{
			Kind:   gqlgenerated.TFTLineupSignalKind(item.Kind),
			Entity: mapTFTEntities([]tft.Entity{item.Entity})[0], HolderUnit: holder,
...
func mapTFTEntities(items []tft.Entity) []*gqlgenerated.TFTEntity {
	...
	for _, item := range items {
		...
		itemClass := gqlgenerated.TFTItemClass(item.ItemClass)
		if itemClass == "" {
			itemClass = gqlgenerated.TFTItemClassUnknown
		}
		out = append(out, &gqlgenerated.TFTEntity{
			ID: item.ID, Name: name, IconURL: iconURL, Cost: cost, ItemClass: itemClass,
		})
	}
	return out
}
