...
type TFTEntity struct {
	...
	ItemClass TFTItemClass `json:"itemClass"`
}
...
type TFTLineup struct {
	...
	Signals                       []*TFTLineupSignal          `json:"signals"`
...
type TFTLineupSignal struct {
	Kind       TFTLineupSignalKind `json:"kind"`
	Entity     *TFTEntity          `json:"entity"`
	HolderUnit *TFTEntity          `json:"holderUnit,omitempty"`
	SampleSize int                 `json:"sampleSize"`
	Rate       float64             `json:"rate"`
}
...
type TFTItemClass string

const (
	TFTItemClassStandard TFTItemClass = "STANDARD"
	TFTItemClassEmblem   TFTItemClass = "EMBLEM"
	TFTItemClassArtifact TFTItemClass = "ARTIFACT"
	TFTItemClassUnknown  TFTItemClass = "UNKNOWN"
)

var AllTFTItemClass = []TFTItemClass{
	TFTItemClassStandard,
	TFTItemClassEmblem,
	TFTItemClassArtifact,
	TFTItemClassUnknown,
}

func (e TFTItemClass) IsValid() bool {
	switch e {
	case TFTItemClassStandard, TFTItemClassEmblem, TFTItemClassArtifact, TFTItemClassUnknown:
		return true
	}
	return false
}

func (e TFTItemClass) String() string {
	return string(e)
}

func (e *TFTItemClass) UnmarshalGQL(v any) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("enums must be strings")
	}
	...
	*e = TFTItemClass(str)
	if !e.IsValid() {
		return fmt.Errorf("%s is not a valid TFTItemClass", str)
	}
	return nil
}

func (e TFTItemClass) MarshalGQL(w io.Writer) {
	fmt.Fprint(w, strconv.Quote(e.String()))
}

func (e *TFTItemClass) UnmarshalJSON(b []byte) error {
	s, err := strconv.Unquote(string(b))
	if err != nil {
		return err
	}
...
func (e TFTItemClass) MarshalJSON() ([]byte, error) {
	var buf bytes.Buffer
	e.MarshalGQL(&buf)
	return buf.Bytes(), nil
}

type TFTLineupSignalKind string

const (
	TFTLineupSignalKindEmblem    TFTLineupSignalKind = "EMBLEM"
	TFTLineupSignalKindArtifact  TFTLineupSignalKind = "ARTIFACT"
	TFTLineupSignalKindThreeStar TFTLineupSignalKind = "THREE_STAR"
	TFTLineupSignalKindAugment   TFTLineupSignalKind = "AUGMENT"
)

var AllTFTLineupSignalKind = []TFTLineupSignalKind{
	TFTLineupSignalKindEmblem,
	TFTLineupSignalKindArtifact,
	TFTLineupSignalKindThreeStar,
	TFTLineupSignalKindAugment,
}

func (e TFTLineupSignalKind) IsValid() bool {
	switch e {
	case TFTLineupSignalKindEmblem, TFTLineupSignalKindArtifact, TFTLineupSignalKindThreeStar, TFTLineupSignalKindAugment:
		return true
	}
	return false
}

func (e TFTLineupSignalKind) String() string {
	return string(e)
}

func (e *TFTLineupSignalKind) UnmarshalGQL(v any) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("enums must be strings")
	}

	*e = TFTLineupSignalKind(str)
	if !e.IsValid() {
		return fmt.Errorf("%s is not a valid TFTLineupSignalKind", str)
	}
	return nil
}

func (e TFTLineupSignalKind) MarshalGQL(w io.Writer) {
	fmt.Fprint(w, strconv.Quote(e.String()))
}

func (e *TFTLineupSignalKind) UnmarshalJSON(b []byte) error {
	s, err := strconv.Unquote(string(b))
	if err != nil {
		return err
	}
...
func (e TFTLineupSignalKind) MarshalJSON() ([]byte, error) {
	var buf bytes.Buffer
	e.MarshalGQL(&buf)
	return buf.Bytes(), nil
}
