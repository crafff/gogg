...
WITH snapshot AS (
    SELECT id, patch, revision
    FROM tft_static_snapshots
    WHERE source = 'cdragon' AND locale = $2 AND status = 'published'
    ORDER BY published_at DESC, id DESC
    LIMIT 1
)
...
func (q *Queries) ListLatestTFTLocalizedStaticObjects(ctx context.Context, objectKinds []string, locale string) ([]ListLatestTFTLocalizedStaticObjectsRow, error) {
	rows, err := q.db.Query(ctx, listLatestTFTLocalizedStaticObjects, objectKinds, locale)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
