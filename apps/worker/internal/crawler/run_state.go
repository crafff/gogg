...
// NewLiteRunState creates a crawler-lite owned run in the DB.
func NewLiteRunState(ctx context.Context, store *storage.Store, profileName *string, profile *config.RunProfile, lastRunEnd time.Time) (*RunState, error) {
	var version *string
	if profile.Version != "" {
		version = &profile.Version
	}
	...
	id, startedAt, err := store.CreateLiteRun(ctx, profileName, sp, lastRunEnd)
	if err != nil {
		return nil, err
	}
	slog.Info("lite_run_created", "run_id", id, "mode", profile.Mode, "tiers", profile.TargetTiers)
	return &RunState{ID: id, Profile: profile, StartedAt: startedAt, LastRunEnd: lastRunEnd, store: store}, nil
}
...
func ResumeRunState(run *storage.Run, profile *config.RunProfile, store *storage.Store, executionOrder []int) *RunState {
	...
	if run.CurrentTier != nil {
		state.CurrentTier = *run.CurrentTier
	}
